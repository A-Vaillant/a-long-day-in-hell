...
import { PERSONALITY, decayBias, entityCompatibility } from "./personality.core.js";
// --- Component keys ---
...
export function socialPressureSystem(world, thresholds = DEFAULT_THRESHOLDS, pressureRate = 0.1, awareness = DEFAULT_AWARENESS) {
    ...
    for (const [entity, pos, psych, ident] of entities) {
        ...
        alive.push({ entity: entity, pos: position, psych: psychology, disp });
    }
    // For each non-mad, non-catatonic entity, count mad entities within shout range
    for (const target of alive) {
        if (target.disp === "mad" || target.disp === "catatonic")
            continue;
