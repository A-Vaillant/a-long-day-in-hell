...
describe("movementSystem", () => {
    ...
    it("dead NPCs do not move", () => {
        const w = createWorld();
        const e = makeNpc(w, { position: 5, alive: false, behavior: "explore" });
        movementSystem(w, makeRng([0.5]));
        const pos = getComponent(w, e, POSITION);
        assert.equal(pos.position, 5);
    });
    ...
    it("explore NPC moves 1 step in heading direction", () => {
        const w = createWorld();
        const e = makeNpc(w, { position: 5, heading: 1, behavior: "explore" });
        movementSystem(w, makeRng([0.5]));
        const pos = getComponent(w, e, POSITION);
        assert.equal(pos.position, 6);
    });
    ...
    it("explore floor change at rest area", () => {
        const w = createWorld();
        const e = makeNpc(w, { position: 9, floor: 3, heading: 1, behavior: "explore" });
    ...
    it("explore no floor change at non-rest area", () => {
        const w = createWorld();
        const e = makeNpc(w, { position: 5, floor: 3, heading: 1, behavior: "explore" });
        movementSystem(w, makeRng([0.01, 0.01, 0.01]));
        const pos = getComponent(w, e, POSITION);
        assert.equal(pos.position, 6); // not a rest area
    ...
    it("seek_rest steps toward nearest rest area", () => {
        ...
        movementSystem(w, makeRng([0.5]));
        const pos = getComponent(w, e, POSITION);
        assert.equal(pos.position, 8); // toward 10
        const mov = getComponent(w, e, MOVEMENT);
    ...
    it("seek_rest steps left toward lower rest area", () => {
        ...
        movementSystem(w, makeRng([0.5]));
        const pos = getComponent(w, e, POSITION);
        assert.equal(pos.position, 2); // toward 0
    });
    ...
    it("batch: explore moves n steps in heading", () => {
        const w = createWorld();
        const e = makeNpc(w, { position: 50, heading: 1, behavior: "explore" });
        movementSystem(w, makeRng([0.5, 0.5, 0.5]), undefined, 20);
        const pos = getComponent(w, e, POSITION);
        assert.equal(pos.position, 70); // 50 + 1*20
    });

    it("batch: wander_mad random walk", () => {
        const w = createWorld();
        const e = makeNpc(w, { position: 50, behavior: "wander_mad" });
        // 10 steps: all 0.3 < 0.5 → all +1 → net +10
        movementSystem(w, makeRng([0.3]), undefined, 10);
        const pos = getComponent(w, e, POSITION);
        assert.equal(pos.position, 60);
    });
